...
class MessageFormatTests: QuickSpec {
    override func spec() {
        it("default format") {
            let expectedFormat = MessageFormat().dateTime.space.string("[").executable.string("]").space.file.string(".").function.string(":").line.space.logLevel.space.emoji.string(":").space
            expect(MessageFormat.default).to(equal(expectedFormat))
        }
        itBehavesLike("format description") {
            ["messageFormat": MessageFormat().dateTime,
             "expected": "#DATE#"]
        }
        itBehavesLike("format description") {
            ["messageFormat": MessageFormat().executable,
             "expected": "#EXEC#"]
        }
        itBehavesLike("format description") {
        ...
        itBehavesLike("format description") {
            ["messageFormat": MessageFormat().function,
             "expected": "#FUNC#"]
        }
        itBehavesLike("format description") {
            ["messageFormat": MessageFormat().line,
             "expected": "#LINE#"]
        }
        itBehavesLike("format description") {
        ...
        itBehavesLike("format description") {
            ["messageFormat": MessageFormat().logLevel,
             "expected": "#LEVEL#"]
        }
        itBehavesLike("format description") {
            ["messageFormat": MessageFormat().space,
             "expected": " "]
        }
        itBehavesLike("format description") {
            ["messageFormat": MessageFormat().string("Hello World!"),
        ...
        itBehavesLike("format description") {
            ["messageFormat": MessageFormat.default,
             "expected": "#DATE# [#EXEC#] #FILE#.#FUNC#:#LINE# #LEVEL# #EMOJI#: "]
        }
    }
}
